...
```
명령형 프로그래밍은 어떻게 할지 표현하고, 선언형 프로그래밍은 무엇을 할 건지 표현
```

<br>

함수형 코드는 명령형 프로그래밍이나 OOP 코드보다 더 간결하고 예측가능하여 테스트하는 것이 쉬움
...
## 2. 함수형 프로그래밍의 의미를 파악하기 전 꼭 알아야 할 것들
...
이는 순수함수가 아님, 이유는 ex 메소드에서 인자가 아닌 condition을 사용했기 때문
...
순수함수로 만들면, 에러를 추적하는 것이 쉬워짐 인자에 문제가 있거나 함수 내부에 문제가 있거나 둘 중 하나일 수 밖에 없기 때문
...
Java 8이 릴리즈되면서, Java에서도 함수형 프로그래밍이 가능해짐
...
여태까지 우리는 Java에서 객체지향 프로그래밍을 할 때 '데이터를 어떻게 처리할 지에 대해 명령을 통해 해결'했음
...
뭐가 다른건지 크게 와닿지 않을 수 있지만, 중요한건 프로그래밍의 패러다임 변화라는 것

단순히 함수를 선언해서 데이터를 내가 원하는 방향으로 처리해나가는 함수형 프로그래밍 방식을 볼 수 있음
